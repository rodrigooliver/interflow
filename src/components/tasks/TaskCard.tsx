...
export function TaskCard({ 
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`bg-white dark:bg-gray-800 p-3 rounded-md shadow-sm border relative overflow-hidden
    ...
    >
      ...
      <div className="flex justify-between items-start mb-2 relative">
        ...
        {canBeCompleted && (onUpdateStatus !== undefined) && (
          ...
          >
            ...
            <span>{t('tasks:markAsCompleted')}</span>
          </button>
        )}
      ...
      <div className="flex justify-between items-start mt-2">
        <div className="flex-1 min-w-0">
          <h3 className={`font-medium text-gray-900 dark:text-white ${isCompleted ? 'line-through text-gray-500 dark:text-gray-400' : ''}`}>
            {task.title}
            {task.is_archived && (
      ...
      <div className={`mt-2 space-y-2 ${isCompleted ? 'text-gray-500 dark:text-gray-400' : ''}`}>
        {/* Due Date */}
        {formattedDueDate && (
          <div className={`flex items-center text-xs ${
            ...
            {isOverdue && (
              <AlertCircle className="w-3.5 h-3.5 ml-1 text-red-500 dark:text-red-400" />
            )}
          </div>
        )}
