...
interface TransferChatToCustomerModalProps {
  chat: Chat;
  onClose: () => void;
  onTransferComplete: (newCustomerId: string) => void;
}

export function TransferChatToCustomerModal({ 
  chat, 
  onClose, 
  onTransferComplete 
}: TransferChatToCustomerModalProps) {
  ...
  const handleTransfer = async () => {
    ...
    try {
      const response = await api.post(`/api/${currentOrganizationMember.organization.id}/chat/${chat.id}/transfer-to-customer`, {
        newCustomerId: selectedCustomer.id
      });

      if (!response.data.success) {
        throw new Error(response.data.error || 'Erro ao transferir atendimento');
      }

      toast.success(t('chats:transfer.singleChatSuccess', 'Atendimento transferido com sucesso'));
      onTransferComplete(selectedCustomer.id);
      onClose();
    } catch (error: unknown) {
      console.error('Erro ao transferir atendimento:', error);
      const errorMessage = error instanceof Error 
        ? error.message 
        : t('chats:transfer.error', 'Erro ao transferir atendimento');
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg">
        <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-medium text-gray-900 dark:text-white">
            {t('chats:transfer.singleTitle', 'Transferir Atendimento')}
          </h2>
          <button
            onClick={onClose}
            ...
            <>
              <div className="mb-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={t('chats:transfer.searchPlaceholder', 'Buscar cliente...')}
              ...
              {loading ? (
                ...
                <div className="text-center text-gray-500 dark:text-gray-400 py-8">
                  {t('chats:transfer.noResults', 'Nenhum cliente encontrado')}
                </div>
              ) : null}
            </>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center space-x-3 p-4 bg-yellow-50 dark:bg-yellow-900/50 rounded-lg">
                ...
                <p className="text-yellow-700 dark:text-yellow-300">
                  {t('chats:transfer.confirmSingle', 'Tem certeza que deseja transferir este atendimento para outro cliente? Esta ação não pode ser desfeita.')}
                </p>
              </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                >
                  ...
                  <span>{t('common:confirm', 'Confirmar')}</span>
                </button>
              </div>
            </div>
          )}
